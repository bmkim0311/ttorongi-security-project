...
ACCOUNT_LOCK_FILE = (
    "/var/lib/ttorongi-security/account_locks.json"
)
...
def get_account_lock(username):
    ...
    lock = locks.get(normalized_username)

    if not isinstance(lock, dict):
        return None

    locked_until_text = lock.get(
        "locked_until"
    )
...
def init_db():
    ...
    # VM 실습용 초기 계정. 공개 저장소에서는 환경변수로 교체 가능하다.
    ...
    if user is None:
        db.execute(
            ...
            (
                "user",
                generate_password_hash(demo_user_password),
                "김또롱",
                "user@ttorongi.com",
                "010-1234-5678",
                "user",
...
def login():
    ...
    if request.method == "POST":
        ...
        account_lock = get_account_lock(
            username
        )

        if account_lock is not None:
            write_event(
                message="반복 로그인 실패로 임시 잠금된 계정의 로그인 시도",
                category="security",
                ...
                request_path=request.path,
                http_method=request.method,
                failure_count=account_lock.get(
                    "failure_count"
                ),
                locked_at=account_lock.get(
                    "locked_at"
                ),
                locked_until=account_lock.get(
                    "locked_until"
                ),
                lock_reason=account_lock.get(
                    "reason"
                ),
            )
            flash(
