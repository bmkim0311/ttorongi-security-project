...
# ===== SCENARIO4_MODE: SECURE START =====
@app.before_request
def protect_partner_export_static_files():
    """
    static/exports 경로의 운영자료를 비로그인 상태에서
    직접 내려받지 못하도록 차단한다.

    정상적인 내보내기 기능은 로그인 세션이 있을 때
    기존 Flask static 응답을 그대로 사용한다.
    """
    protected_prefix = (
        "/static/exports/"
    )
    ...
    write_event(
        ...
        category="security",
        event_type=(
            "unauthorized_partner_export_access"
        ),
        severity="warning",
        result="failure",
        source_ip=get_source_ip(),
        requested_export_path=request.path,
        request_path=request.path,
        http_method=request.method,
...
def partner_bicycle_status_change():
    # SCENARIO5_MODE: SECURE_AUDIT_RBAC
    validate_csrf()

    source_ip = get_source_ip()

    actor_user_id = session.get(
        "partner_user_id"
    )

    actor_username = session.get(
        "partner_username"
    )
    ...
    try:
        ...
        # 현재 협력업체에 배정된 활성 작업만 허용한다.
        assigned_task = db.fetchone(
            """
            SELECT
                id,
                task_code,
