...
def create_unsigned_qr_token(
    ...
    # SCENARIO2_MODE: VULNERABLE
    """
    자전거 ID와 대여소 ID만 Base64로 인코딩한다.

    랜덤 인증값이 없고 서버의 현재 토큰과도 비교하지
    않으므로 공격자가 값을 위조하거나 재사용할 수 있다.
    """
    token_payload = (
        f"{int(bicycle_id)}:{int(station_id)}"
    )
...
def decode_unsigned_qr_token(
    ...
    if bicycle_id <= 0 or station_id <= 0:
        return None
...
def get_bicycle_preview_by_code(code_text):
    """Base64 QR 토큰과 기존 자전거 번호를 구분하여 미리보기한다."""
    submitted_code = str(code_text or "").strip()
...
def qr_rental():
    ...
    try:
        ...
        if decoded_qr is not None:
            ...
            #
            # Base64에서 추출한 bicycle_id와 station_id만
            # 확인한다.
            #
            # 전자서명 검증 없음
            # 만료시간 검증 없음
