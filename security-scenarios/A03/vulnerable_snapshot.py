...
def qr_rental():
    ...
    try:
        ...
        # SCENARIO3_MODE: VULNERABLE_RACE_WINDOW
        # A03 Race Condition 실습용 취약 로직
        #
        # 동일 사용자의 활성 대여 상태를 잠금 없이 읽는다.
        # 동시 요청은 같은 초기 상태를 확인할 수 있다.
        active_rental = get_active_rental(
            g.user["id"],
            db=db,
            for_update=False,
        ...
        # A03 RACE CONDITION WINDOW
        #
        # 자전거 조회와 상태 변경 사이에 잠금이나 재검증이
        # 없으며, 의도적인 대기 시간 동안 동시 요청이
        # 같은 available 상태를 확인할 수 있다.
        write_event(
            message=(
                "A03 경쟁 구간 진입 "
                f"pid={os.getpid()} "
